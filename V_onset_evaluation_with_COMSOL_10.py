...
plots_base_dir = os.path.join(folder_path, f"Plots - {base_name} - CapRadius_Sweep - CapArea based onset")
...
r_cap_list = [35e-9, 20e-9, 10e-9] #in meters

r_inner_fixed = 40e-9
cap_depth_fixed = 1e-3
...
for r_cap in r_cap_list:
    ...
    if prev_onset_ray is None:
        ...
        v_low = 50
        v_high = max(400, 2 * prev_onset_ray) 
    ...
    while (v_high - v_low) > tol:
        ...
        # Find the mainimum Electric Field directly from the exported data
        # e_min_data = df_ray_eval['E'].min()
    ...
    detailed_summary_data.append({
        'R_cap (nm)': r_cap * 1e9,
        'V_onset_Rayleigh_entire_cap_area (V)': v_onset_rayleigh,
