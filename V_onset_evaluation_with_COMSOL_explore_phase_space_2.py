...
for idx, current_params in enumerate(run_list):
    ...
    try:
        ...
        base_half_pitch_m = base_half_pitch_um * 1e-6  # Convert to meters
        print(f'base_half_pitch extracted natively: {base_half_pitch_m}')
        ...
        if base_half_pitch_m >= 1000e-6:
            n_plate = max(1, int(round(base_half_pitch_m / (target_size_m*1.75))))
            
        if base_half_pitch_m >= 100e-6 and base_half_pitch_m < 1000e-6:
            n_plate = max(1, int(round(base_half_pitch_m / (target_size_m*1.3))))
            
        if base_half_pitch_m < 100e-6:
            n_plate = max(1, int(round(base_half_pitch_m / (target_size_m))))
            
        if cap_depth >= 1000e-6:
            n_cap = max(1, int(round(cap_depth / (target_size_m*1.75))))
            
        if cap_depth >= 100e-6 and cap_depth < 1000e-6:
