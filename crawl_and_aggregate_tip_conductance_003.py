...
def parse_notes_file(file_path):
    """
    Reads a single JSON or TXT file and extracts standard fields.
    """
    data = {"Heat": "N/A", "Pull": "N/A", "Velocity": "N/A", "Delay": "N/A"}
    
    if file_path.endswith('.json'):
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                content = json.load(f)
                # Handle list of dicts (as per your template)
        ...
        try:
            ...
            # Note: Parsing complex nested recipes from TXT is harder; 
            # this assumes basic string extraction if needed.
        except Exception as e:
...
def get_hierarchical_metadata(tip_folder_path):
    """
    1. Look in Tip Folder (JSON -> TXT)
    2. Look in Parent Folder (JSON -> TXT)
    """
    final_data = {
