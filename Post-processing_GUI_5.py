# -*- coding: utf-8 -*-
"""
Created on Mon Mar 30 13:00:24 2026
...
def load_data():
    ...
    for entry in master_log:
        ...
        # Robust parser that ignores spaces
        def parse_val(v):
...
class PlotDashboard:
    ...
    def build_ui(self):
        ...
        ttk.Radiobutton(f0, text="E-field vs Angle (Single Run/Group)", variable=self.plot_mode, value="E-field vs Angle", command=self.update_dynamic_ui).grid(row=1, column=0, sticky='w')
        ...
        # Save specific widgets to variables so they can be toggled
        self.chk_rayleigh = tk.Checkbutton(f4, text="Plot Point Rayleigh", variable=self.plot_rayleigh)
        self.chk_rayleigh.grid(row=0, column=0, sticky='w')
        
        self.chk_taylor = tk.Checkbutton(f4, text="Plot Taylor Limit", variable=self.plot_taylor)
        self.chk_taylor.grid(row=1, column=0, sticky='w')
        
        tk.Checkbutton(f4, text="Auto-Save as PDF", variable=self.auto_save, fg="blue").grid(row=2, column=0, sticky='w', pady=2)
        tk.Checkbutton(f4, text="Normalize to Global Max", variable=self.normalize_var, fg="green").grid(row=3, column=0, sticky='w', pady=2)
        
        self.chk_area = tk.Checkbutton(f4, text="Plot Area-Based Rayleigh", variable=self.plot_area_rayleigh)
        self.chk_area.grid(row=0, column=1, sticky='w', padx=5)
        
        diam_frame = tk.Frame(f4)
        diam_frame.grid(row=1, column=1, sticky='w', padx=5)
        ...
        margin_frame.grid(row=2, column=1, sticky='w', padx=5)
    ...
    def update_dynamic_ui(self, event=None):
        ...
        # Dynamic disabling based on mode
        if mode == "E-field vs Angle":
            ...
            self.lbl_diam.config(state="disabled") # Greys out the text visually
        else:
    ...
    def _load_meniscus_df(self, row):
        ...
        try:
            ...
            print(f"Error reading spatial file for {run_name}: {e}")
            return None

    def calc_area_onset(self, row, target_radius_nm):
        df_meniscus = self._load_meniscus_df(row)
    ...
    def generate_plot(self):
        ...
        if mode == "E-field vs Angle":
            ...
            def collect_efield_row(row, label_suffix, color_idx):
                ...
                if df_meniscus is not None:
                    ...
                    all_efield_maxes.append(np.max(e_fields))
                    plot_items.append({'angles': angles, 'e_fields': e_fields, 'label': f"E-field {label_suffix}", 'color_idx': color_idx})
            
            if g_col == "None":
                df_subset = base_df.head(10) if len(base_df) > 10 else base_df
                if len(base_df) > 10: messagebox.showwarning("Warning", "Over 10 runs match. Showing the first 10 to avoid clutter.")
                ...
                for g_val in selected_groups:
                    ...
                    if not g_df.empty:
                        ...
                        unit_str = f" {unit}" if unit else ""
                        collect_efield_row(row, f"({g_col}={g_val}{unit_str})", color_idx)
                        color_idx += 1

            global_max = 1.0
            if self.normalize_var.get() and all_efield_maxes:
                global_max = max(all_efield_maxes)
                if global_max == 0: global_max = 1.0
                
            for item in plot_items:
                c = colors[item['color_idx'] % len(colors)]
                self.ax.plot(item['angles'], item['e_fields'] / global_max, linestyle='-', color=c, label=item['label'])

            self.ax.set_title("Electric Field vs Meniscus Angle", fontsize=12, pad=15)
            self.ax.set_xlabel("Angle (deg)")
            self.ax.set_ylabel("Normalized Electric Field" if self.normalize_var.get() else "Electric Field (V/m)")
            safe_x = "Angle"
            ...
            if self.normalize_var.get():
                ...
                if active_cols:
                    global_max = base_df[active_cols].max().max()
                    if pd.isna(global_max) or global_max == 0: global_max = 1.0
            ...
            def plot_onset_series(df_slice, label_suffix, color_idx):
                ...
                if self.plot_rayleigh.get():
                    self.ax.plot(df_slice[x_col], df_slice['V_onset_rayleigh'] / global_max, marker='o', linestyle='-', color=c, label=f"Point Rayleigh {label_suffix}")
                if self.plot_taylor.get():
                    self.ax.plot(df_slice[x_col], df_slice['V_onset_taylor'] / global_max, marker='s', linestyle='--', color=c, label=f"Taylor {label_suffix}")
                if self.plot_area_rayleigh.get() and 'V_onset_area' in df_slice.columns:
                    if not df_slice['V_onset_area'].isna().all():
                        self.ax.plot(df_slice[x_col], df_slice['V_onset_area'] / global_max, marker='^', linestyle=':', color=c, label=f"Area Rayleigh (d={target_diam}nm) {label_suffix}")
            ...
            if g_col == "None":
                ...
                for g_val in selected_groups:
                    ...
                    if not g_df.empty:
                        unit = self.param_units.get(g_col, "")
                        unit_str = f" {unit}" if unit else ""
                        plot_onset_series(g_df, f"({g_col}={g_val}{unit_str})", color_idx)
                        color_idx += 1
            ...
            self.ax.set_ylabel("Normalized Onset Voltage" if self.normalize_var.get() else "Onset Voltage (V)")
            safe_x = x_col.replace("/", "_")

        self.ax.set_xscale("linear" if mode == "E-field vs Angle" else self.x_scale.get())
        ...
        if self.auto_save.get():
            ...
            prefix = "EField_vs" if mode == "E-field vs Angle" else "Onset_vs"
