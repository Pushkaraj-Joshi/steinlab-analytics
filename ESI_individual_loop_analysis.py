...
base_dir = r"G:\Shared drives\Stein Lab Team Drive\Pushkaraj\ESI characterization\July_25_2025\Tip_02"
...
for idx, data_entry in enumerate(processed_data):\
    ...
    min_y = min(np.min(I), np.min(y_data))
    max_y = max(np.max(I), np.max(y_data))
    ...
    ax2.errorbar(x_data, y_data, x_error, y_error,
                fmt=':o', color = 'red', linewidth=3, markersize=8, capsize=2,
                label=f'loop {loop_number}')

    ax2.plot(np.linspace(100, 200, 100), np.ones(100)*threshold_current*1e12,
